...
function poserQuestions() {
  ...
  <input id="maxHeuresSemaine" type="number" min="8" max="30" value="18">
...
function applySpacedRevisionPlan(pool, scores) {
  ...
  topSubjects.forEach((matiere) => {
    revisions.push({ matiere, offset: 1 });
    revisions.push({ matiere, offset: 3 });
    revisions.push({ matiere, offset: 7 });
  });

  // On ajoute plusieurs occurrences pour favoriser la répétition espacée.
  revisions.forEach((r) => pool.push(r.matiere));
}
...
function genererEmploiPersonnel() {
  ...
  freeSlots.forEach((slot) => {
    ...
    if (sessionsDay[slot.jour] >= maxSessions) {
  ...
  lastConfig = { classe, maxSessions, maxHeuresSemaine, examMode, prefs };
  saveState();

  afficherTable(calendar);
  afficherStats(calendar);
  detecterSurcharge(calendar, maxSessions, maxHeuresSemaine);
  afficherBadgeMotivation(calendar, maxSessions, maxHeuresSemaine);
}

function detecterSurcharge(calendar, maxSessions, maxHeuresSemaine) {
  let totalStudy = 0;
  const details = [];
...
function afficherBadgeMotivation(calendar, maxSessions, maxHeuresSemaine) {
  ...
  jours.forEach((j) => {
    ...
    if (count > 0 && count <= maxSessions) joursEquilibres += 1;
  });
